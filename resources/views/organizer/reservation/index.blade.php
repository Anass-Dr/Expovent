<x-app-layout>
    ...
    <section class="event__popup-area">
        <div class="popup__wrapper">
            <div class="popup__title-wrapper">
                <h3 class="popup__title">Validate Reservation</h3>
            </div>
            <div class="popup__input-wrapper">
                <form action="{{ route('reservation.update') }}" method="post">
                    ...
                    <div class="singel__input-field mb-15">
                        ...
                        <input name="event_name" id="event_name" type="text" readonly/>
                    </div>
                    <div class="singel__input-field mb-15">
                        <label class="input__field-text">Reservation Status</label>
                        ...
                        <div class="breadcrumb__menu">
                            <nav>
                                <ul>
                                    ...
                                    <li class="active"><span>Reservations List</span></li>
                                </ul>
                            </nav>
                        </div>
                    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="status">
                                                <div class="attendant__status">
                                                    @php
                                                        $color = '';
                                                        if ($reservation->status == 'approved') $color = 'bg-green';
                                                        if ($reservation->status == 'rejected') $color = 'bg-warn';
                                                        if ($reservation->status == 'pending') $color = 'warning-bg';
                                                    @endphp
                                                    <span class="status__tag {{ $color }}">{{ $reservation->status }}</span>
                                                </div>
                                            </td>
                                            <td>
                                                    ...
                                                    <div class="card__header-dropdown">
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-list">
                                                                <a class="dropdown__item event__popup-active"
                                                                   href="#">Edit</a>
                                                            </div>
                                                        </div>
                                                    </div>
    ...
    <script>
        document.querySelectorAll('.event__popup-active').forEach(btn => btn.addEventListener('click', (e) => {
            const tr = e.target.closest('tr');
            const id = tr.querySelector('input[type="hidden"]').value;
            const status = tr.querySelector('td.status span').textContent;
            const title = tr.querySelector('td.title span').textContent;
            const modal = document.querySelector('.event__popup-area');
            modal.querySelector('#reservation_id').value = id;
            modal.querySelector('#event_name').value = title;
            modal.querySelectorAll('select option').forEach(option => option.selected = option.value === status);
        }));
    </script>
</x-app-layout>
